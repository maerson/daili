...
def download_index():
    ...
    for c in [ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '0']:
        ...
        while cur_num <= total:
            ...
            # 保存页面
            fw = open("d:\\lexico.com\\"+c+"_"+str(cur_num)+".html", 'w', -1, encoding="utf_8_sig")
            fw.write(html)
            fw.close()       
            
            # 获取总的页数
            # <a href="/list/b/39">Last »</a>
            # 
            if cur_num ==1:
                m = re.search(u'href="/list/[a-z0]/(?P<page_num>[0-9]+)">Last &raquo;',html)
